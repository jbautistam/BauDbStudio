...
namespace Bau.Libraries.ComicsReader.ViewModel.Reader
{
	...
	public class ComicContentViewModel : BaseObservableObject, PluginsStudio.ViewModels.Base.Interfaces.IDetailViewModel
	{
		...
		{
			return CanGoPage(ComicPages.Items.Count - 1);
		}
		...
		{
			return CanGoPage(GetActualPage() + 1);
		}
		...
		{
			if (ComicPages.SelectedItem != null)
				return (ComicPages.SelectedItem as BookPageViewModel).Page - 1;
			else
				return 0;
		}
