...
namespace Bau.Libraries.EbooksReader.ViewModel.Reader
{
	...
	public class EBookContentViewModel : BaseObservableObject, PluginsStudio.ViewModels.Base.Interfaces.IDetailViewModel
	{
		...
		// Variables privadas
		private string _fileName, _tempPath;
		...
		{ 
			...
			TreePages.PropertyChanged += (sender, args) => 
												{
													if (args.PropertyName.Equals(nameof(TreePages.SelectedNode)))
		...
		/// </summary>
		private bool CanGoLastPage()
		{
			return CanGoPage(_bookNodePages.Count - 1);
		}
		...
		{
			return _bookNodePages.Count > 0 && page >= 0  && page < _bookNodePages.Count;
		}
		...
		public string GetUrlPage()
		{
			string url = (TreePages.SelectedNode as Explorer.EbookNodeViewModel)?.Url;
				...
				if (!string.IsNullOrWhiteSpace(url))
					return "file://" + System.IO.Path.Combine(_tempPath, url).Replace("\\", "/");
				else
					return string.Empty;
		}
		...
		/////		Página seleccionada
		///// </summary>
		//public Explorer.EbookNodeViewModel SelectedPage
		//{
		//	get { return _selectedPage; }
		//	set 
		//	{ 
		//		if (CheckObject(ref _selectedPage, value))
		//			RaiseEventChangePage();
		//	}
		//}
