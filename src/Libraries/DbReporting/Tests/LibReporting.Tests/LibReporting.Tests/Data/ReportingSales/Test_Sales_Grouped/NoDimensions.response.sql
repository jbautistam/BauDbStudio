...
WITH
	...
	SalesCte AS 
		(SELECT [SalesAnalysis].[SalesQuantity], [SalesAnalysis].[SalesAmountTaxesIncluded], [SalesAnalysis].[SalesAmountTaxesExcluded]
			FROM [Fact].[SalesAnalysis] INNER JOIN CalendarCte
				ON [SalesAnalysis].[Date] = [CalendarCte].[Date]
			WHERE [SalesAnalysis].[Refund] = 0
		)
	SELECT SUM(SalesQuantity) AS Quantity, SUM(SalesAmountTaxesIncluded) AS AmountTaxesIncluded, 
