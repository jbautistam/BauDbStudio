...
namespace Bau.Libraries.LibReporting.Application.Controllers.Queries
{
	...
	/// </summary>
	internal class QueryDimensionGeneratorOld : BaseQueryGenerator
	{
		internal QueryDimensionGeneratorOld(ReportQueryGenerator generator, List<DimensionModel> dimensionsRequested)
		...
		private QueryModel GetChildQuery(DimensionModel dimension)
		{
			QueryModel query = new QueryModel(dimension.GlobalId, QueryModel.QueryType.Dimension, dimension.GlobalId);
				...
				// Asigna los campos
				foreach (BaseColumnRequestModel baseColumnRequest in Generator.Request.DimensionColumns)
					if (IsDimensionColumn(dimension, baseColumnRequest))
						AddColumn(query, (baseColumnRequest as DimensionColumnRequestModel).ColumnId, string.Empty, baseColumnRequest);
				// Devuelve la consulta
				return query;
		}
		...
		{
			return baseColumnRequest is DimensionColumnRequestModel dimensionRequest && 
