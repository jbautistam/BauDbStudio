...
namespace Bau.Libraries.LibReporting.Application.Controllers.Queries
{
	...
	{
		internal QueryExpressionsGenerator(ReportQueryGenerator generator, List<DimensionModel> dimensionsRequested)
		...
		internal QueryModel GetQuery()
		{
			...
			ReportDataSourceModel reportDataSource = Generator.Report.Expressions[0];
			...
			QueryModel query = new QueryModel(baseDataSource.GlobalId, QueryModel.QueryType.Expressions, "tmpExpressions");
				...
				ComputeFieldsGroupBy(query, DimensionsRequested, reportDataSource);
		...
		/// </summary>
		private void ComputeFieldsGroupBy(QueryModel query, List<DimensionModel> dimensionRequested, ReportDataSourceModel dataSource)
		{
			foreach (DimensionModel dimension in dimensionRequested)
		...
		{
			foreach (BaseColumnRequestModel baseColumn in request.Columns)
				if (baseColumn is ExpressionRequestModel requestColumn)
					foreach (DataSourceColumnModel column in dataSource.Columns)
						if (requestColumn.ColumnId.Equals(column.ColumnId, StringComparison.CurrentCultureIgnoreCase))
							AddColumn(query, column.ColumnId, string.Empty, requestColumn.AggregatedBy, baseColumn);
		}
	}
}