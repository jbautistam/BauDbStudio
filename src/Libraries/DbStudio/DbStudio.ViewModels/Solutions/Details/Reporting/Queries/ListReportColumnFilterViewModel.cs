		...
		private bool ValidateData(ListItemReportColumnFilterViewModel filter)
		{
			...
			FilterRequestModel.ConditionType condition = filter.GetSelectedCondition();
				...
				if (condition == FilterRequestModel.ConditionType.Undefined)
					ReportViewModel.ReportingSolutionViewModel.SolutionViewModel.MainViewModel.MainController.HostController.SystemController.ShowMessage("Seleccione la condición");
				else if (condition == FilterRequestModel.ConditionType.Contains && string.IsNullOrWhiteSpace(filter.Value1?.ToString()))
					ReportViewModel.ReportingSolutionViewModel.SolutionViewModel.MainViewModel.MainController.HostController.SystemController.ShowMessage("Seleccione el valor");
				else 
					validated = true;
