...
namespace Bau.Libraries.DbStudio.ViewModels.Solutions.Details.Reporting.Relations
{
	...
	{
		// Variables privadas
		private ObservableCollection<DimensionRelationViewModel> _listRelations;
		...
		private void OpenRelation(DimensionRelationViewModel viewModel)
		{
			bool isNew = viewModel == null;
				...
				if (ReportingSolutionViewModel.SolutionViewModel.MainViewModel.MainController.OpenDialog(viewModel) == BauMvvm.ViewModels.Controllers.SystemControllerEnums.ResultType.Yes)
				{
		...
		{
			if (SelectedItem != null && ReportingSolutionViewModel.SolutionViewModel.MainViewModel.MainController.HostController.SystemController.ShowQuestion("¿Desea eliminar esta relación?"))
			{
				// Borra la relación
		...
		internal bool ValidateData()
		{
			bool validated = false;
				...
				if (ListRelations.Count == 0)
					ReportingSolutionViewModel.SolutionViewModel.MainViewModel.MainController.HostController.SystemController.ShowMessage("No se ha definido ninguna dimensión");
				else
				{
							...
							DataSourceColumnModel relatedColumn = foreignKeyViewModel.GetRelatedColumn();
								...
								if (relatedColumn != null)
									relation.ForeignKeys.Add(new RelationForeignKey
																		{
																			ColumnId = foreignKeyViewModel.SourceColumn.ColumnId,
																			TargetColumnId = relatedColumn.ColumnId
																		}
															);
						}
